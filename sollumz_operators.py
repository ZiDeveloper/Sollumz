    ...
    def export_object(self, obj):
        try:
            ...
            if obj.sollum_type == SollumType.DRAWABLE:
                ...
                valid_type = True
            elif obj.sollum_type in SollumType.YMAP:
                filepath = self.get_filepath(
                    obj.name.lower(), YMAP.file_extension)
...
def sollumz_menu_func_import(self, context):
    self.layout.operator(SOLLUMZ_OT_import.bl_idname,
                         text=f"Codewalker XML({YDR.file_extension}, {YDD.file_extension}, {YFT.file_extension}, {YBN.file_extension}, {YCD.file_extension}, {YMAP.file_extension})")
